...
for category in ["pathmnist", "dermamnist", "bloodmnist"]:
    ...
    for i, depth in enumerate([18, 34, 50]):
        data = pd.read_csv(f"data/resnet-choice/train-acc/baseline-{category}-{depth}_version_0.csv")

        # Take every 20th value
        steps = data["Step"].values[::20]
        values = data["Value"].values[::20]
